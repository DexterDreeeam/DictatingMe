...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%~dp0assets\run-package.ps1" -DebugBuild
set "EXIT_CODE=%ERRORLEVEL%"

if not "%EXIT_CODE%"=="0" (
  echo.
  echo [DictatingMe] Debug package build or silent installation failed with code %EXIT_CODE%.
) else (
  echo.
  echo [DictatingMe] Debug package build and silent installation completed successfully.
  echo [DictatingMe] Logs: %%LOCALAPPDATA%%\DictatingMe\logs\debug
)
